...
export const getJokeFromExternalApi: RequestHandler<{}, {}/*GetAllJokesResponseBody*/> = async (
    req,
    res
  ) => {
    
    const response = await fetch("https://v2.jokeapi.dev/joke/Any?type=single&idRange=1-300", {method: 'GET'}); //oder POST
    ...
    createdJoke = await jokeRepository.findOneOrFail({titel: "test", text: jokes.joke}).catch(async () => {
        return await jokeRepository.save(newJoke).catch(() => {});
    });

    res.send({status: "ok", joke: createdJoke});
